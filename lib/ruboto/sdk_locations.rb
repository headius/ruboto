...
module Ruboto
  module SdkLocations
    ...
    unless File.exists? "#{ANDROID_HOME}/platform-tools"
      abort "The '<ANDROID_HOME>/platform-tools' directory is missing.
Please set the ANDROID_HOME environment variable to a proper Android SDK installation."
    end
    ANDROID_TOOLS_REVISION = File.read("#{ANDROID_HOME}/platform-tools/source.properties").slice(/Pkg.Revision=\d+/).slice(/\d+$/).to_i
